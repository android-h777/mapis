								...
								label =	$(
										( '<label class="filename">' +
											'<span class="MultiFile-label" title="' + t + '">'+
												'<span class="MultiFile-title link">'+ n +'</span>'+
...
*/						
					...
					//var fileSize = $('<td class="fileSize"/>');
					var fileSize = [];
					$.each(files, function (i, file) {
						var v = String(file.name || '' ),
								label =	$(
										(
//											'<td class="fileSize">' +
//											'<span class="MultiFile-label">'+sl(file.size || 0)+'</span>' +
//											'</td>'
											''
										)
								);
						// append file label to list
						//if(i>1) fileSize.append(', ');
						fileSize[i] = label;
/*
...
*/
					...
					$.each(files, function (i, file) {
						...
						var
							r = $('<span class="MultiFile-label hoverable-2"></span>'),
							b = $('<a class="MultiFile-remove" href="#' + MultiFile.wrapID + '" alt="remove">' + MultiFile.STRING.remove + '</a>')
